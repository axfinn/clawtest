            ...
            <div v-show="activeTab === 'files'" class="tab-content">
              ...
              <div v-else class="files-layout">
                ...
                <div class="file-tree">
                  ...
                  <!-- Truncated notice -->
                  <div v-if="filesTruncated" class="px-3 py-1.5 text-xs text-amber-400 bg-amber-400/10">
                    仅显示 {{ taskFiles.length }} / {{ filesTotal }} 个文件，请使用搜索过滤
                  </div>
...
async function submitTask() {
  ...
  try {
    ...
    if (activeMode.value === 'ask') {
      ...
      if (activeMode.value !== 'export' || resumeTaskId.value) body.module = newTask.value.module
...
async function loadResultForTask() {
  ...
  filesTotal.value = data.total || taskFiles.value.length
  filesTruncated.value = data.truncated || false
